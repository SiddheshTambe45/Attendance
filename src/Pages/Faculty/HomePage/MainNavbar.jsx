        ...
        <div className='mainNavbar' id='mainNavbar'>
            <div className='container-fluid px-5'>
                <div className='row navbar-row'>
                    <div className='nav-1 col col-lg-3'>
                        <Link className="text-decoration-none d-flex justify-content-start align-items-center" to='/' style={{ height: '100%' }}>
                            <img src={logo} className='img-fluid img-logo' alt='' />
                        </Link>
                    </div>

                    <div className='nav-2 col col-lg-7 d-flex align-items-center justify-content-center'>
                        <div className='sentence-display'>
                            {sentences[sentenceIndex] === 'www.siesgst.edu.in' ? (
                                <Link href={`http://${sentences[sentenceIndex]}`} className='text-decoration-none text-dark display-xl-4 display-6' target="_blank" rel="noopener noreferrer">
                                    {sentences[sentenceIndex]}
                                </Link>
                            ) : (
                                <h4 className='display-xl-4 display-6 text-center'>{sentences[sentenceIndex]}</h4>
                            )}
                        </div>
                    </div>

                    <div className='nav-3 col col-lg-2'>
                        <div className='menu-btn d-flex justify-content-end align-items-center' style={{ height: '100%' }}>
                            <button className='border-0 bg-transparent' onClick={() => setMenuOption(!menuOption)}>
                                {menuOption ? (
                ...
                <div className={`row homePageHiddenNav ${menuOption ? 'hidden' : 'visible'}`}>
                    <div className='d-flex justify-content-end align-items-center'>
                        <div className='px-5'>
                            <Link to='/faculty/' className='display-6 text-decoration-none text-dark'>
                                Home
                            </Link>
                        </div>

                        <div className='px-5'>
                            <Link to='/faculty/attendanceUpdate' className='display-6 text-decoration-none text-dark'>
                                Attendance Update
                            </Link>
                        </div>

                        <div className='px-5'>
                            <Link to='/faculty/reportPage' className='display-6 text-decoration-none text-dark'>
                                Report Page
                            </Link>
                        </div>
                    </div>
