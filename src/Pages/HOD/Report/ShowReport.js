...
const ShowReport = ({ attendance, loading, error }) => {
  ...
  const filteredAttendance = combinedAttendance.filter((student) => {
    ...
    if (filter !== 'default') {
      const attendancePercentages = Object.values(student.subjects);

      // Determine the worst attendance percentage across subjects
      const minAttendance = Math.min(...attendancePercentages.map((percentage) => parseFloat(percentage)));
  ...
  return (
    <div className='ShowReportHOD' id='ShowReportHOD'>
            ...
            <tr className="subjects-row">
              ...
              {subjects.map((subject, index) => (
                <th key={index} className="text-center">
                  {subject} ({attendance[0].subjects[subject]?.type || ''})
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filteredAttendance.map((student) => (
              <tr key={student.prn}>
                ...
                {subjects.map((subject, index) => (
                  <td key={index} className="text-center subject-data">
