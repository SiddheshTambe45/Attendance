...
const Allocation = () => {
  ...
  useEffect(() => {
    const fetchCriteria = async () => {
      try {
        ...
        const response = await axiosInstance.get('/hod/getCriteriaFacSub', {
          params: { department: hodDepartment }
        });
        const { semesters, divisions, batches } = response.data;
        setSemesters(semesters || []);
  ...
  const fetchSubjectsAndFaculty = async () => {
    try {
      ...
      const response = await axiosInstance.get('/hod/getSubjectsAndFaculty', {
        params: { branch: hodDepartment, semester: currentSemester, division: selectedDivision, batch: selectedBatch }
      });
      const { faculty, subjects, allDepartments } = response.data;
      ...
      // Update selectedOptions with pre-assigned faculty
      const preAssignedOptions = subjects.reduce((options, subject) => {
        if (subject.FACULTY_ID) {
          options[subject.SUBJECT_ID] = subject.FACULTY_ID;
        }
        return options;
      }, {});
      setSelectedOptions(preAssignedOptions);
    } catch (error) {
      console.error('Error fetching subjects and faculty:', error);
      alert('Failed to fetch subjects and faculty.');
    }
  ...
  const fetchFacultyByDepartment = async (department) => {
    ...
    try {
      ...
      const response = await axiosInstance.get('/hod/getFacultyByDepartment', {
        params: { department }
      });
  ...
  const applyChanges = async () => {
    try {
      ...
      await axiosInstance.post('/hod/allocateFacultyToSubject', dataToSend);
  ...
  const getAvailableTeachers = (index) => {
    ...
    return teachers.filter(teacher => {
      const subject = subjects[index];
      return !selectedFacultyIds.has(teacher.FACULTY_ID) || teacher.FACULTY_ID === selectedOptions[subject.SUBJECT_ID];
    });
  };
    ...
    <div className="container mt-5">
      ...
      <div className="row mb-3">
        ...
        <div className="col-12 col-md-3 d-flex align-items-center">
          <button className="btn btn-primary" onClick={fetchSubjectsAndFaculty} style={{marginTop: '28px', marginLeft: '118px'}}>Get Data</button>
        </div>
      </div>
